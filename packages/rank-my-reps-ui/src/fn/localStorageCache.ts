...
function isCacheItem<T>(item: any, guard: (z: any) => z is T): item is CacheItem<T> {
    // Structure matches
    return typeof item === "object"
        ...
        // Validate expiry
        && (item.expirySeconds < 0 || (new Date(item.date + (100*item.expirySeconds)) > new Date()))
        // Cached item matches expected type
        && guard(item.value);
}

export async function storeValue<T>({key, value, expirySeconds = 3000}: CacheItemParam<T>): Promise<void> {
...
const isCustomStorageEvent = (ev: Event & { detail?: CustomStorageEventData }): ev is CustomEvent<CustomStorageEventData> => {
    return typeof ev.detail === "object"
        && typeof ev.detail.key === "string"
        && typeof ev.detail.newValue === "string"
        && (typeof ev.detail.oldValue === "string" || ev.detail.oldValue === undefined || ev.detail.oldValue === null);
};
...
export async function getValue<T>(key: string, guard: (x:any) => x is T): Promise<CacheResponse<T>> {
    const storage = getStorage();
...
export function checkHit<T>(response: CacheResponse<T>): response is CacheHit<T> {
...
export function cacheGet<P extends any[],R>(prefix: string, validFn: Validator<R>, fn:OrigFn<P,R>): OrigFn<P,R> {
...
export function cacheSet<R,V,K extends string[],P extends any[]>(prefix: string, toKeyParts: (...args: P) => K, toValue: (...args: P) => V, fn:OrigFn<P,R>, expirySeconds:number = 3000): OrigFn<P,R> {
    return async (...args: P): Promise<R> => {
...
export async function* cacheSubscribe<R>(key: string, validFn: Validator<R>): AsyncIterableIterator<R> {
