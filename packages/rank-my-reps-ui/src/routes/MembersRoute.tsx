...
type Position<T extends RollCallVote> = T extends { positions: Array<infer P> } ? P : never;
interface MemberAndPosition { member: CongressMember, position: {good: number, bad: number} }
...
interface MemberVotes { [vote: string]: Position<RollCallVote>}
function getVotes(votes: MemberVotes, scores: ScoreCard|undefined, good: boolean): number {
    if(!scores) { return 0; }
    return Object.entries(votes).filter(([key,value]) => {
            return scores[key] && value && voteMatchesPosition(value.vote_position, good && (scores[key].support));
        }).length;
}
const getRouteComponent = ({ match }) => {
            ...
            try {
                ...
                for(const score of scores) {
                    try {
                        ...
                        if(passageVote) {
                            const vote = await getRollCallVote(chamber, congress, getBillSession(bill.introduced_date), Number(passageVote.roll_call));
                            fetches.push(vote);
                        }
                    } catch (e) {
                        console.log(e);
    ...
    React.useEffect(() => {
        if(loading || scoresLoading || votesLoading) { return; }
        setJoinedLoading(true);
        try {
            const tmpJoined: MemberAndPosition[] = [];
            for(const member of members) {
                const memberVotes: MemberVotes = votes.reduce((a,v) => ((a[v.bill.bill_id] = v.positions.find((p) => p.member_id === member.id)),a), {});
        ...
        columns={[
            {
                ...
                render: (item: MemberAndPosition) => {
                    const { short_title, first_name, middle_name, last_name, suffix} = item.member;
                    return [short_title, first_name,middle_name,last_name,suffix].filter((e) => e).join(' ')
                },
                title: 'Name'
            },
            ...
            { field: 'member.state', title: 'State' },
            {
                customSort: (row1: MemberAndPosition, row2: MemberAndPosition) => {
                    return Number(row2.member.seniority) - Number(row1.member.seniority);
                },
...
export function getMembersTitle(congress: number, chamber: Chamber) {
    const formatChamber = chamber.charAt(0).toUpperCase() + chamber.slice(1).toLowerCase();
    return `${Nth(congress)} ${formatChamber}`;
}

export default <T extends RouteProps>(props: T): JSX.Element => <Route {...props} component={getRouteComponent}/>;