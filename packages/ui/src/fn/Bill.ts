...
function isAbstractBill(
  data: any
): data is AbstractBill & { [B: string]: unknown } {
  return (
    isObject(data) &&
    ...
    (isString(data.gpo_pdf_uri) ||
      data.gpo_pdf_uri === null ||
      data.gpo_pdf_uri === undefined) &&
    isString(data.congressdotgov_url) &&
    isString(data.govtrack_url) &&
    isString(data.introduced_date) &&
    isBoolean(data.active) &&
    (isString(data.last_vote) ||
      data.last_vote === null ||
      data.last_vote === undefined) &&
    (isString(data.house_passage) ||
      data.house_passage === null ||
      data.house_passage === undefined) &&
    (isString(data.senate_passage) ||
      data.senate_passage === null ||
      data.senate_passage === undefined) &&
    (isString(data.enacted) ||
      data.enacted === null ||
      data.enacted === undefined) &&
    (isString(data.vetoed) ||
      data.vetoed === null ||
      data.vetoed === undefined) &&
    isNumber(data.cosponsors) &&
    (data.cosponsors_by_party === null ||
      data.cosponsors_by_party === undefined ||
      (isObject(data.cosponsors_by_party) &&
        Object.entries(data.cosponsors_by_party).every(([, value]) =>
          isNumber(value)
        ))) &&
    isString(data.committees) &&
    Array.isArray(data.committee_codes) &&
    data.committee_codes.every((v: any) => isString(v)) &&
    Array.isArray(data.subcommittee_codes) &&
    data.subcommittee_codes.every((v: any) => isString(v)) &&
...
export function isBill(data: any): data is Bill {
  return isAbstractBill(data) && isString(data.sponsor_name);
}
